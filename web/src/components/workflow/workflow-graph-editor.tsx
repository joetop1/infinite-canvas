...
import type { WorkflowFieldMapping, WorkflowGraphPreview } from "@/lib/workflow-channel";
...
export function WorkflowGraphEditor({ workflowJson, workflowGraph, fields, onChange, disabled = false, emptyDescription = "尚未读取到工作流拓扑" }: WorkflowGraphEditorProps) {
    ...
    const enabledFieldCountByNode = useMemo(() => {
        const counts = new Map<string, number>();
        enabledFields.forEach((field) => counts.set(String(field.nodeId), (counts.get(String(field.nodeId)) || 0) + 1));
        return counts;
    }, [enabledFields]);
                        ...
                        <svg className="workflow-graph-svg" width={graph.width * scale} height={graph.height * scale} viewBox={`0 0 ${graph.width} ${graph.height}`}>
                            <g>
                                 ...
                                 {graph.nodes.map((node) => {
                                     const fieldCount = enabledFieldCountByNode.get(node.id) || 0;
                                    return (
                                        <g
                                            key={node.id}
                                            data-workflow-node
