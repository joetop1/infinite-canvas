...
import { runWorkflowTask, type WorkflowGenerationTask } from "@/services/api/workflow-generation";
...
export function WorkflowTestWorkbench({ workflowRef, token, provider, workflowId, workflowKind = "workflow", title, capability, fields, disabled = false, disabledReason, onBeforeTest }: WorkflowTestWorkbenchProps) {
    ...
    const [result, setResult] = useState<WorkflowGenerationTask | null>(null);
    ...
    const runTest = async () => {
        ...
        try {
            ...
            const response = await runWorkflowTask(token, {
                ...
                size: config.size,
                count: Number(config.canvasImageCount) || 1,
                ...
                referenceImages: await Promise.all(files.image.slice(0, imageReferenceCount).map((item) => readFileAsDataUrl(item.file, `读取素材失败：${item.file.name}`))),
                referenceVideos: await Promise.all(files.video.map((item) => readFileAsDataUrl(item.file, `读取素材失败：${item.file.name}`))),
                referenceAudios: await Promise.all(files.audio.map((item) => readFileAsDataUrl(item.file, `读取素材失败：${item.file.name}`))),
                mask: maskEnabled && files.image[imageReferenceCount] ? await readFileAsDataUrl(files.image[imageReferenceCount].file, `读取素材失败：${files.image[imageReferenceCount].file.name}`) : undefined,
                source: "workflow_settings_test",
            }, controller.signal, (task) => {
                if (abortRef.current !== controller) return;
            ...
            const detail = reason instanceof Error ? reason.message : "工作流测试失败";
            setError(detail);
            setStage("测试失败");
...
function WorkflowParameter({ field, value, onChange }: { field: WorkflowFieldMapping; value: unknown; onChange: (value: unknown) => void }) {
    ...
    return (
        <label>
            ...
            {choices.length ? (
                <Select showSearch value={value} options={choices.map((option) => ({ label: workflowOptionText(option), value: workflowOptionValue(option) }))} onChange={onChange} />
...
function ResultPreview({ capability, urls }: { capability: WorkflowCapability; urls: string[] }) {
...
function uniqueParameterFields(fields: WorkflowFieldMapping[]) {
    return fields.filter((field) => !isPromptOrMediaSource(String(field.source || "")));
}

function testFieldKey(field: WorkflowFieldMapping) {
    return `field:${field.nodeId}:${field.fieldName}`;
}
...
function capabilityName(value: WorkflowCapability) {
