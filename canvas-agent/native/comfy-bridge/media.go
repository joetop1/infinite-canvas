...
func collectResult(store *recoveryStore, mode string, history jsonMap) (jsonMap, error) {
	outputs := make([]outputItem, 0)
	root, _ := mapValue(history["outputs"])
	for _, nodeID := range sortedNodeIDs(root) {
		collectOutputItems(root[nodeID], "", &outputs)
	}
	if len(outputs) == 0 {
	...
	for _, item := range outputs {
		resultID, err := newLocalOutputID()
		if err != nil {
			return nil, fmt.Errorf("生成 ComfyUI 本地结果 ID 失败：%w", err)
		}
		mimeType := mimeForName(item.Filename)
		if err := store.saveLocalOutput(localOutput{
			ResultID:  resultID,
			Filename:  item.Filename,
			Subfolder: item.Subfolder,
			Type:      firstNonEmpty(item.Type, "output"),
		}); err != nil {
			return nil, fmt.Errorf("保存 ComfyUI 本地结果索引失败：%w", err)
		}
		value := jsonMap{
			"url":      bridgeOutputURL(resultID),
			"resultId": resultID,
			"mimeType": mimeType,
			"name":     item.Filename,
			"local":    true,
	...
	return nil, errors.New("ComfyUI 没有返回可用产物")
}
...
func outputKind(value string) string {
	...
	if stringIn(extension, ".png", ".jpg", ".jpeg", ".webp", ".gif", ".bmp", ".avif") {
		return "image"
	}
	return "other"
}
